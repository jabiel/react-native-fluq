...
- (void)handleAttributionData:(NSNotification *)notification
{
  ...
  if (params.count > 0) {
    ...
    [defaults setObject:linkData forKey:@"RNFluqAttributionData"];
                  ...
                  reject:(RCTPromiseRejectBlock)reject)
{
  ...
  NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
  NSDictionary *attributionData = [defaults objectForKey:@"RNFluqAttributionData"];
  
  if (attributionData) {
    // Clear the stored attribution data after reading
    [defaults removeObjectForKey:@"RNFluqAttributionData"];
    [defaults synchronize];
